...
function VippsReturInnhold() {
  ...
  useEffect(() => {
    ...
    fetch('/api/vipps/callback', {
      ...
      body: JSON.stringify({ reference: ref, varer: ordre.varer, kunde: ordre.kunde }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.status === 'ok') {
  ...
  }, [searchParams, tømKurv]);

  if (status === 'behandler') {
    return (
      <div className="mx-auto max-w-2xl px-6 py-32 text-center">
        <div className="mx-auto w-12 h-12 rounded-full border-4 border-stone-200 border-t-tre-600 animate-spin mb-8" />
        <h1 className="font-serif text-3xl font-bold text-stone-800">Bekrefter betalingen …</h1>
        <p className="mt-3 text-stone-500">Vent litt mens vi verifiserer Vipps-betalingen.</p>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-6 py-32 text-center">
        ...
        <p className="mt-4 text-stone-500 text-lg leading-relaxed">
          Vipps-betalingen er registrert og du vil motta en bekreftelse på e-post.
        </p>
        <p className="mt-2 text-stone-400 text-sm">Estimert levering: 3–5 virkedager via Posten.</p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-32 text-center">
      ...
      <h1 className="font-serif text-4xl font-bold text-stone-800">
        {avbrutt ? 'Betalingen ble avbrutt' : 'Noe gikk galt'}
      </h1>
      <p className="mt-4 text-stone-500 text-lg leading-relaxed">
        {avbrutt
          ? 'Vipps-betalingen ble ikke fullført. Handlekurven din er beholdt.'
