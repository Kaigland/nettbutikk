    ...
    <div className="group rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
      <Link href={`/produkter/${produkt.id}`}>
        ...
        <div className="p-4 pb-2">
          <p className="text-xs uppercase tracking-wider text-stone-400 capitalize">{produkt.kategori}</p>
          <h3 className="mt-1 font-serif text-lg font-semibold text-stone-800 leading-snug">{produkt.navn}</h3>
          <p className="mt-1 text-tre-700 font-semibold">{produkt.pris.toLocaleString('nb-NO')} kr</p>
        </div>
      </Link>
      <div className="px-4 pb-4 pt-2">
        ...
        >
          {utsolgt ? 'Utsolgt' : 'Legg i handlekurv'}
        </button>
      </div>
    </div>
  );
